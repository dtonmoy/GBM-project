...
# PARAMETERS
fgseaDir <- file.path("../fgsea_TCGA_single")
dir.create(fgseaDir, recursive = TRUE, showWarnings = FALSE)

rankedGenesList <- readRDS(file.path("../data/survival/TCGA_single_rankedGenesList.rds"))

ann.clin <- readRDS(file.path("../data/survival/TCGA-GBM_clinical.rds"))
...
# PLOT
ggmat <- data.frame(Surv = ann.clin$days_to_death[keep] / 30,
                    Score = nesSum[keep])
...
# RECURRENT DEAD PATIENT
keep <- ann.sample$Sample.name == "Recurrent Solid Tumor" & !is.na(ann.clin$days_to_death)
