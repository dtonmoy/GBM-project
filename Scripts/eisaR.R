...
# COUNT
setwd(file.path("../Data/quasR/"))
cntEx <- read.csv("Final_count_exon.csv")
rownames(cntEx) <- cntEx[,1]
cntEx <- cntEx[, -1]

cntIn <- read.csv("Final_count_intron.csv")
...
# SAMPLE ANNOTATION
ann.sample <- read.csv("Annotation_col.csv")
...
geneMat <- read.xlsx(file.path("../Data/quasR/", "Six cat_Leading edge genes.xlsx"))
...
# LOAD GENES OF INTEREST
geneMat <- read.xlsx(file.path("../Data/quasR/", "Six cat_Leading edge genes.xlsx"))
geneMat <- geneMat[geneMat$Genes %in% degList[[1]]$symbol, ]
